    ...
    {analysis && <div className="modalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setAnalysis(null); }}>
      <section className="companyReview" role="dialog" aria-modal="true" aria-labelledby="company-review-title">
        ...
        <div className="reviewBody">
          ...
          <div><h3>Matches your priorities</h3>{analysis.matched_priorities.length > 0 ? <div className="priorityChecklist">{analysis.matched_priorities.map((item) => <span className="matched" key={item}>✓ {item}</span>)}</div> : <p>This company isn&apos;t tagged for any of your selected priorities in Green Canopy&apos;s classification data.</p>}<small>Green Canopy score is a transparent educational alignment measure based on classification tags, not a third-party rating.</small></div>
        </div>
